...
class LocalOptimaNetwork:
    ...
    def local_minimization(self, initial_point: np.ndarray) -> tuple[np.ndarray, float, float]:
        result = minimize(
    ...
    def add_node(self, solution: np.ndarray, value: float, rounded_value: float) -> None:
        self.graph.add_node(self.preprocess_solution(solution), value=value, rounded_value=rounded_value)
    ...
    def add_or_update_edge(self, solution1: np.ndarray, solution2: np.ndarray) -> None:
        ...
        if self.graph.has_edge(preprocessed_solution1, preprocessed_solution2):
            ...
            self.graph.add_edge(preprocessed_solution1, preprocessed_solution2, weight=1.0)

    def run_basin_hopping(self) -> None:
        initial_point = np.random.uniform(self.bounds[:, 0], self.bounds[:, 1])
        current_solution, current_value, rounded_value = self.local_minimization(initial_point)
        if not self.has_node(current_solution):
            self.add_node(current_solution, value=current_value, rounded_value=rounded_value)
        ...
        while iterations_since_last_update < self.stopping_threshold:
            ...
            new_solution, new_value, new_rounded_value = self.local_minimization(new_solution)

            if new_value <= current_value:
                if self.has_node(new_solution):
                    self.add_or_update_edge(current_solution, new_solution)
                else:
                    self.add_node(new_solution, value=new_value, rounded_value=new_rounded_value)
                    self.add_or_update_edge(current_solution, new_solution)
    ...
    def compress_graph(self) -> nx.DiGraph:
        ...
        for solution1, solution2, data in self.graph.edges(data=True):
            ...
            if solution1_fitness == solution2_fitness:
                continue
            solution1_compressed = tuple(sorted([node for node in compressed_graph if solution1 in node][0]))
            solution2_compressed = tuple(sorted([node for node in compressed_graph if solution2 in node][0]))
            if compressed_graph.has_edge(solution1_compressed, solution2_compressed):
                compressed_graph[solution1_compressed][solution2_compressed]["weight"] += data["weight"]
            else:
                compressed_graph.add_edge(solution1_compressed, solution2_compressed, weight=data["weight"])
